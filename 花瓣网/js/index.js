(function(){
	...
	window.onscroll=function(){
		...
		var bodyTop = document.documentElement.scrollTop||document.body.clienTop;
		if(bodyHeight - bodyTop <= 1000){
			autoCreateImg();
			waterFull("dom_pull", "box");
			tabHeight();
		}
		...
		if(bodyTop >= 180){
			...
			startMove(TopNav,{opacity:100})
		}else{
			startMove(TopNav,{opacity:0},null,function(){
...
function tabHeight(){
	var bgHeight = document.getElementById("tab_content");
	var box = bgHeight.getElementsByClassName("box")
	bgHeight.style.height = box[box.length - 1].offsetTop + box[box.length - 1].offsetHeight + 200 + "px";
	// console.log(box[29].offsetHeight)
};
